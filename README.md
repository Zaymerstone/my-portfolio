# 🌐 Personal Portfolio Website

My portfolio website designed to showcase my work as a aspiring **full-stack developer** with a growing interest in **Machine Learning and AI development**.  
This site serves as my professional online presence for internships, collaborations, and other opportunities.

---

## 🚀 Overview

This portfolio highlights:

- My full-stack projects (React, Flask, Node.js, PostgreSQL, etc.)
- My academic background in **Cognitive Science (MSc, CogSUP Programme)**
- Clean and responsive UI
- Direct access to GitHub, LinkedIn, and other contact information

Built with a focus on:

- Clear demonstration of technical skills
- Presenting myself as a developer

---

## 🎯 Key Features

- **Responsive design** — optimized for desktop, mobile and tablets
- **Project showcase section** with summaries, tech stack, and links
- **About Me page** with academic background & career direction
- **Clean UI/UX** with structured layout and minimalistic styling
- **Contact section** with email, LinkedIn, and GitHub integration
- **Fast and lightweight** static frontend

---

## 🛠️ Tech Stack

### **Frontend**

- React
- TypeScript
- Tailwind CSS

### **Deployment**

- GitHub
- Vercel

---

### 👤 Author

**Nesterov Egor**  
MSc in Cognitive Science  
Software Engineer Intern | Full-stack Developer Intern

**Contact:**

- 📧 Email: [egor.nesterov2002@gmail.com](mailto:egor.nesterov2002@gmail.com)
- 🔗 LinkedIn: [linkedin.com/in/enesterov22](https://www.linkedin.com/in/enesterov22/)
- 💬 Telegram: [t.me/Zaymerstone](https://t.me/Zaymerstone)
